import type { FastifyInstance } from 'fastify';
export declare function registerProductRoutes(app: FastifyInstance): Promise<void>;
export declare function registerOrderRoutes(app: FastifyInstance): Promise<void>;
export declare function registerStudentRoutes(app: FastifyInstance): Promise<void>;
export declare function registerAdminRoutes(app: FastifyInstance): Promise<void>;
export declare function registerCustomPapersRoutes(app: FastifyInstance): Promise<void>;
export declare function registerEnhancedCoursesRoutes(app: FastifyInstance): Promise<void>;
export declare function registerAnalyticsRoutes(app: FastifyInstance): Promise<void>;
export declare function registerNewsletterRoutes(app: FastifyInstance): Promise<void>;
export declare function registerAllRoutes(app: FastifyInstance): Promise<void>;
//# sourceMappingURL=routes.d.ts.map
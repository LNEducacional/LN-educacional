...
class AutoReplyService {
  ...
  async processAutoReply(message: MessageData): Promise<boolean> {
  ...
  private async selectTemplate(message: MessageData): Promise<AutoReplyTemplate | null> {
  ...
  private findTemplateByKeywords(message: MessageData): AutoReplyTemplate | null {
    const messageText = `${message.subject} ${message.message}`.toLowerCase();

    for (const template of this.templates.values()) {
      if (!template.enabled || !template.keywords) continue;
  ...
  private findTemplateByCategory(category: string): AutoReplyTemplate | null {
    for (const template of this.templates.values()) {
  ...
  private async sendAutoReply(message: MessageData, template: AutoReplyTemplate): Promise<void> {
  ...
  private processTemplate(content: string, message: MessageData): string {
  ...
  private generateAutoReplyHTML(content: string, message: MessageData): string {
    return `
...
Equipe Educacional LN`,
  ...
  async addTemplate(template: Omit<AutoReplyTemplate, 'id'>): Promise<string> {
    const id = `custom_${Date.now()}`;
    this.templates.set(id, { ...template, id });
    return id;
  }

  async updateTemplate(id: string, updates: Partial<AutoReplyTemplate>): Promise<boolean> {
  ...
  async updateConfig(newConfig: Partial<AutoReplyConfig>): Promise<void> {
...
export const autoReplyService = new AutoReplyService(autoReplyConfig);

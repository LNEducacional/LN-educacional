...
export async function registerNewsletterRoutes(app: FastifyInstance) {
    ...
    async (request, reply) => {
      try {
        ...
        const result = await emailService.sendNewsletter(
          subscribers.map(s => ({ email: s.email, name: s.name || undefined, id: s.id })),
          newsletterData
        );
    ...
    async (request, reply) => {
      try {
        ...
        const [notifications, total] = await Promise.all([
          prisma.postNotification.findMany({
            where,
            include: {
              post: {
                select: {
          ...
          prisma.postNotification.count({ where }),
        ]);
  ...
  const leadSubscribeSchema = z.object({
  ...
  app.post('/leads/subscribe', async (request, reply) => {
    try {
      ...
      const existingLead = await prisma.order.findFirst({
        where: {
